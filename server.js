...
const REFRESH = Number(process.env.REFRESH_SECONDS || 1);
...
const TELEGRAM_ALERT_CHAT_ID = String(process.env.TELEGRAM_ALERT_CHAT_ID || process.env.TELEGRAM_CHAT_ID || '-1003869687513').trim();
const TELEGRAM_ALERT_THREAD_ID = Number(process.env.TELEGRAM_ALERT_THREAD_ID || 7);
const ALERT_COOLDOWN_SECONDS = Number(process.env.ALERT_COOLDOWN_SECONDS || 300);
const ALERT_RAM_PCT = Number(process.env.ALERT_RAM_PCT || 85);
const ALERT_DISK_PCT = Number(process.env.ALERT_DISK_PCT || 85);
const ALERT_LOAD_PER_CORE = Number(process.env.ALERT_LOAD_PER_CORE || 2.0);
...
function buildHealth(rp, dp, lpc) {
  const alerts = [];
  if (rp >= ALERT_RAM_PCT) alerts.push(`RAM ${rp}%`);
  if (dp >= ALERT_DISK_PCT) alerts.push(`DISK ${dp}%`);
  if (lpc >= ALERT_LOAD_PER_CORE) alerts.push(`LOAD ${lpc}/core`);
  const level = rp >= 90 || dp >= 90 ? 'danger' : alerts.length ? 'warn' : 'ok';
  const label = level === 'danger' ? 'CRITICAL' : level === 'warn' ? 'ATTENTION' : 'HEALTHY';
  return { level, label, alerts };
}

async function sendTelegramAlert(vpsName, metrics) {
  const alerts = metrics?.health?.alerts || [];
  if (!TG_TOKEN || !TELEGRAM_ALERT_CHAT_ID || !alerts.length) return;
  const key = `${vpsName}:${alerts.join('|')}`;
  const now = Date.now();
  if (now - (alertState.get(key) || 0) < ALERT_COOLDOWN_SECONDS * 1000) return;
  alertState.set(key, now);
  const tag = ALLOWED_TG_USER_ID ? `<a href="tg://user?id=${ALLOWED_TG_USER_ID}">user</a>` : '';
  const text = `${tag} 🚨 <b>${escHtml(metrics.health.label)}</b> ${escHtml(vpsName)}\n${alerts.map(escHtml).join('\n')}\nCPU ${metrics.cpu.pct}% • RAM ${metrics.ram.pct}% • DISK ${metrics.disk.pct}% • SWAP ${metrics.swap?.pct ?? 0}%`;
  await telegram('sendMessage', {
    chat_id: TELEGRAM_ALERT_CHAT_ID,
    message_thread_id: TELEGRAM_ALERT_THREAD_ID || undefined,
    parse_mode: 'HTML',
...
async function readMemInfo() {
  ...
  const m = Object.fromEntries(raw.split('\n').map(l => {
    ...
    return x ? [x[1], Number(x[2]) * 1024] : null;
  }).filter(Boolean));
  ...
  return { total, available, swaptotal, swapused: Math.max(0, swaptotal - swapfree), swapfree };
}

async function readDisk() {
  try {
    const { stdout } = await execFileAsync('df', ['-B1', '--output=size,used,avail', '/'], { timeout: 1200, maxBuffer: 64 * 1024 });
    const [size, used, available] = stdout.trim().split('\n').at(-1).trim().split(/\s+/).map(Number);
    return { size: size || 1, used: used || 0, available: available || 0 };
  } catch {
    return { size: 1, used: 0, available: 0 };
  }
}
...
async function readCpuPct() {
  try {
    ...
    if (!cpuPrev) { cpuPrev = { idle, total }; return 0; }
    ...
    return +Math.min(100, (os.loadavg()[0] / cores) * 100).toFixed(1);
  }
}

async function topProcesses(procs) {
  if (Date.now() - topCache.ts < 3000 && topCache.data.length) return topCache.data;
  try {
    ...
    if (rows.length) {
      topCache = { ts: Date.now(), data: rows };
      return rows;
    }
  } catch {}
...
async function serviceStatus() {
  if (Date.now() - servicesCache.ts < 10000 && servicesCache.data.length) return servicesCache.data;
  ...
  servicesCache = { ts: Date.now(), data: out };
  return out;
}

async function buildLocalMetrics() {
  const [mem, disk, cp, top, services] = await Promise.all([readMemInfo(), readDisk(), readCpuPct(), topProcesses(), serviceStatus()]);
  const cores = os.cpus().length || 1;
...
async function localMetrics() {
  const now = Date.now();
  if (metricsCache.data && now - metricsCache.ts < 850) return metricsCache.data;
  if (metricsCache.pending) return metricsCache.pending;
  metricsCache.pending = buildLocalMetrics().then(data => {
...
app.get('/api/metrics', async (req, reply) => {
  ...
  if (t.type === 'local') {
    const data = await localMetrics();
    sendTelegramAlert(t.name, data).catch(err => app.log.warn(`alert failed: ${err.message}`));
    return data;
  }
  try {
